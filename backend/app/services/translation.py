...
caching, fallback mechanisms, and error handling for multilingual support using OpenAI API.
"""
...
class OpenAITranslateClient:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise TranslationProviderError(
                "openai", f"OpenAI API key not configured: {str(e)}"
            )
    ...
    async def detect_language(self, text: str) -> LanguageDetectionResponse:
        """Detect language of the given text using OpenAI."""
        start_time = time.time()

        try:
            prompt = f"""Detect the language of the following text and respond with ONLY the ISO 639-1 language code (e.g., 'en' for English, 'hi' for Hindi, 'ta' for Tamil).
...
Language code:"""
            ...
            raise TranslationProviderError(
                "openai", f"Language detection failed: {str(e)}", e
            )
    ...
    async def translate_text(self, request: TranslationRequest) -> TranslationResponse:
        ...
        try:
            ...
            if request.source_language:
                prompt = f"""Translate the following text from {source_lang_name} to {target_lang_name}. Provide ONLY the translation without any explanations or additional text.

Text to translate: {request.text}

Translation:"""
            else:
                prompt = f"""Translate the following text to {target_lang_name}. Provide ONLY the translation without any explanations or additional text.
...
Translation:"""
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a professional translator. Translate text accurately to {target_lang_name}. Respond only with the translation, no explanations.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
            ...
            if not detected_source:
                try:
                    ...
                    detected_source = detection.detected_language
                except:
            ...
            raise TranslationProviderError("openai", f"Translation failed: {str(e)}", e)
...
class TranslationService:
    ...
    def __init__(self):
        ...
        # Note: Cache initialization is async and should be called separately
        # Call await translation_service.initialize_cache() after creating the service
    ...
    def _initialize_clients(self):
        ...
        try:
            # Initialize OpenAI client
            self.clients[TranslationProvider.OPENAI.value] = OpenAITranslateClient()
            logger.info("OpenAI translation client initialized successfully")
        except Exception as e:
            logger.warning(f"OpenAI translation client not available: {e}")
            logger.info("Translation service will work with limited functionality")
            # Don't raise - allow service to start without translation
    ...
    async def detect_language(self, text: str) -> LanguageDetectionResponse:
        ...
        # Try OpenAI client
        if TranslationProvider.OPENAI.value in self.clients:
            try:
                client = self.clients[TranslationProvider.OPENAI.value]
    ...
    async def translate_text(
        ...
        """
        Convenience method to translate text and return just the translated string.
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for provider_name, client in self.clients.items():
            try:
                if provider_name == TranslationProvider.OPENAI.value:

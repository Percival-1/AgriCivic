...
def test_error_handling_in_endpoints(mock_ivr_service):
    ...
    # Also need to mock initialize_services
    mock_ivr_service.initialize_services = Mock(return_value=None)
    mock_ivr_service.weather_service = None
    mock_ivr_service.market_service = None
    ...
    # The error response should be in the response
    mock_ivr_service._generate_error_response.assert_called_once()


def test_post_response_handling():
    """Test post-response handling without mocking."""
    # Test repeat option (goes to main menu)
    response = client.post(
    ...
    assert any(word in response.text for word in ["धन्यवाद", "Thank you", "Hangup"])
